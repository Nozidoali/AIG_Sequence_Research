...
Solution WHY_NodeRewrite( Rwr_Man_t * p, Cut_Man_t * pManCut, Abc_Obj_t * pNode, int fUpdateLevel, int fUseZeros, int fPlaceEnable, int minGain )
{
    int fVeryVerbose = 0;
    Dec_Graph_t * pGraph;
...
Abc_Obj_t * WHY_RandNode( Abc_Ntk_t * pNtk ) {
    ...
    while ( true ) {
        ...
        if ( pObj == NULL || !Abc_ObjIsNode( pObj ) )
            continue;
        if ( Abc_ObjIsCi( pObj ) || Abc_ObjIsCo( pObj ) ) {
            continue;
        }
...
Vec_Ptr_t * WHY_FindNeighbours( Abc_Obj_t * pObj ) {

    // create an empty vector to store the nodes
    Vec_Ptr_t * vObjs = Vec_PtrAlloc( 10 );

    // pObj is the target node
    int i;
    Abc_Obj_t * pFanin;

    Abc_ObjForEachFanin( pObj, pFanin, i ) {
        if ( Abc_ObjIsCi( pFanin ) || Abc_ObjIsCo( pFanin ) ) {
            continue;
        } 
        if ( !Abc_ObjIsNode( pFanin ) || Abc_AigNodeIsAnd( pFanin ) ) {
            continue;
        }

        // push the node into the vector
        Vec_PtrPush( vObjs, pFanin );

        // find the fanin of the fanin nodes
        int ii;
        Abc_Obj_t * pFaninin;

        Abc_ObjForEachFanin( pObj, pFaninin, ii ) {
            if ( Abc_ObjIsCi( pFaninin ) || Abc_ObjIsCo( pFaninin ) ) {
                continue;
            } 
            if ( !Abc_ObjIsNode( pFaninin ) || Abc_AigNodeIsAnd( pFaninin ) ) {
                continue;
            }

            // continue if the node is already added
            if ( Vec_PtrFind( vObjs, pFaninin ) == -1 ) {
                continue;
            }

            // push the node into the vector
            Vec_PtrPush( vObjs, pFaninin );

        }

    }

    // insert all the fanouts
    Abc_Obj_t * pFanout;

    Abc_ObjForEachFanout( pObj, pFanout, i ) {
        if ( Abc_ObjIsCi( pFanout ) || Abc_ObjIsCo( pFanout ) ) {
            continue;
        } 
        if ( !Abc_ObjIsNode( pFanout ) || Abc_AigNodeIsAnd( pFanout ) ) {
            continue;
        }

        // push the fanout node into the vector
        Vec_PtrPush( vObjs, pFanout );
    }
